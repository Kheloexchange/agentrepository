...
// One background watermark image + a few drifting icons per supergroup section.
const SUPERGROUP_DECOR = {
  Sports: { bg: 'assets/cat-sports.svg', icons: ['\u{1F3CF}', '\u26BD', '\u{1F3C6}'] },
  Entertainment: { bg: 'assets/cat-entertainment.svg', icons: ['\u{1F3AC}', '\u{1F3AD}', '\u2728'] },
  Racing: { bg: 'assets/cat-racing.svg', icons: ['\u{1F3C1}', '\u{1F3CE}\uFE0F', '\u{1F3CD}\uFE0F'] },
};
...
function renderGrid() {
  ...
  SUPERGROUPS.forEach((sg) => {
    const catsInGroup = visibleCategories.filter((c) => c.supergroup === sg);
    if (catsInGroup.length === 0) return;

    const tone = toneClass(sg);
    const decor = SUPERGROUP_DECOR[sg];

    const section = document.createElement('section');
    section.className = `supergroup-section tone-${tone}`;

    const bg = document.createElement('div');
    bg.className = 'supergroup-bg';
    bg.setAttribute('aria-hidden', 'true');
    if (decor) bg.style.backgroundImage = `url('${decor.bg}')`;
    section.appendChild(bg);

    const decorWrap = document.createElement('div');
    decorWrap.className = 'supergroup-decor';
    decorWrap.setAttribute('aria-hidden', 'true');
    (decor ? decor.icons : []).forEach((icon) => {
      const span = document.createElement('span');
      span.className = 'decor-el';
      span.textContent = icon;
      decorWrap.appendChild(span);
    });
    section.appendChild(decorWrap);

    const cardsWrap = document.createElement('div');
    cardsWrap.className = 'supergroup-cards';

    catsInGroup.forEach((cat) => {
      const heading = document.createElement('div');
      heading.className = 'shelf-heading';
      heading.innerHTML = `<span class="shelf-dot" style="background:var(--${tone})"></span>${cat.icon} ${cat.label}`;
      cardsWrap.appendChild(heading);
      ...
      SITES.filter((s) => s.category === cat.id)
        ...
        .forEach((site) => {
          ...
          cardEl.classList.add(`tone-${tone}`);
...
// ---------------------------------------------------------------------------
// Hero banner slider — crossfades between the three background slides
// ---------------------------------------------------------------------------
...
function initHeroSlider() {
  ...
  if (reduceMotion) return; // leave the first slide static

  let current = 0;
  setInterval(() => {
    slides[current].classList.remove('is-active');
    current = (current + 1) % slides.length;
    // restart the Ken Burns animation on the newly active slide
    slides[current].style.animation = 'none';
    void slides[current].offsetWidth; // force reflow to restart the animation
...
// ---------------------------------------------------------------------------
// Hero title typography effect — splits "TRENDINGBLOGS" into staggered letters
// ---------------------------------------------------------------------------

function initHeroTitleEffect() {
  const title = document.getElementById('heroTitle');
  if (!title) return;

  const splitTextNode = (el) => {
  ...
  // Letter-split only the lead-in word; the accent word ("BLOGS") keeps its
  // text node intact since the shimmer effect relies on background-clip:text.
  const plainNodes = Array.from(title.childNodes).filter((n) => n.nodeType === Node.TEXT_NODE);
  const accentEl = title.querySelector('.hero-title-accent');

  let globalIndex = 0;
  plainNodes.forEach((textNode) => {
    const wrapper = document.createElement('span');
    wrapper.textContent = textNode.textContent;
    title.replaceChild(wrapper, textNode);
    splitTextNode(wrapper);
    wrapper.querySelectorAll('.letter').forEach((l) => l.style.setProperty('--li', globalIndex++));
    wrapper.replaceWith(...wrapper.childNodes);
  });

  if (accentEl) {
    accentEl.style.setProperty('--li', globalIndex + 2);
  }

  title.classList.add('is-split');

  const totalDelayMs = globalIndex * 45 + 700;
  window.setTimeout(() => title.classList.add('is-settled'), totalDelayMs);
}
