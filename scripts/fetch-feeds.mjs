...
async function main() {
  ...
  // Build a "Top 10" list per group (Sports / Entertainment) by round-robining
  // across that group's categories (each already newest-first), so e.g. Top
  // 10 Sports isn't just 10 cricket stories — it pulls one at a time from
  // Cricket, Football, Tennis, Kabaddi, etc. until 10 are collected.
  function topForGroup(groupName, limit = 10) {
    const categoriesInGroup = [...new Set(sources.filter((s) => s.group === groupName).map((s) => s.category))];
    const queues = categoriesInGroup.map((cat) => (byCategory[cat] || []).slice());
    const picked = [];
    let anyLeft = true;
    while (picked.length < limit && anyLeft) {
      anyLeft = false;
  ...
  const top = {
    Sports: topForGroup('Sports'),
    Entertainment: topForGroup('Entertainment'),
  };
